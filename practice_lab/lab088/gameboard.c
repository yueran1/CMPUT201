...
int gameboard::load_game(const char *filename) {
  ...
  for (int i=0; i<9;i++)
  {
    ...
    {
      fprintf(stderr, "cant read the file %s\n",filename);
      return 10;
    }
    board[i]=helploading(boardspace[0]);
  }
...
int gameboard::save_game(const char *filename) {
  ...
  if (!fp)
  {
    fprintf(stderr,"cant open the %s \n", filename);
    return 10;
  }
  for (int i=0; i<9; i++)
  {
    ...
    {
      fprintf(stderr,"cant write to the file %s\n", filename);
      return 10;
    }
    ...
    {
      if(fprintf(fp,"\n")<0){
        fprintf(stderr, "cant write the %s \n", filename);
        return 10;
      }
    }
  }
