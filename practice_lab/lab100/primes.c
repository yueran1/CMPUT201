...
void *prime_thread(void *data)
{
  ...
  for (qie=job->kaidao;qie<job->end; qie += job->xiayidao  )
  {
    qiewan=qie+ job-> xiayidao;
    if (qiewan>job->end)
    {
      qiewan=job->end;
    }
    ...
    for(uint4 i=job->kaidao;i<job->end;i++)
    {
      if (is_prime(i)){
        job->sum +=1;
...
uint4 num_primes(uint4 a, uint4 b, int tn)
{
  ...
  Job jobs[tn];
  const int daochang = 200;
  const int xiayidao = daochang * tn;
  ...
  for (int i=0; i < tn; ++i) {
    ...
    jobs[i].kaidao=daochang*i;
    jobs[i].daochang=daochang;
    jobs[i].xiayidao=xiayidao;
    jobs[i].end=b+1;
