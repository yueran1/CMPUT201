...
void *prime_thread(void *data)
{
  ...
  uint4 sliceend;
  job->sum=0;
  for (slice=job->start; slice<job->end; slice +=job->nextslice)
  {
    sliceend=slice+job->nextslice;
    if (sliceend>job->end)
    {
      sliceend=job->end;
    }

    for (uint4 i=slice; i<job->end; i++)
    {
      if (is_prime(i))
      {
        job->sum +=1;
