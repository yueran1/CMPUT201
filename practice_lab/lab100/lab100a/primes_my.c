...
void *prime_thread(void *data)
{
  ...
  for (slice= job->start; slice<job->end; slice =slice+job->step)
  {
    sliceEnd= slice+job->slength;
    if (sliceEnd> job->end)
    {
      sliceEnd= job->end;
    }
...
uint4 num_primes(uint4 a, uint4 b, int tn)
{
  ...
  Job jobs[tn];
  const int slength=250;
  const int step= slength *tn;
  ...
  for (int i=0; i < tn; ++i) {
    ...
    jobs[i].start=slength*i;
    jobs[i].slength=slength;
    jobs[i].step=step;
    jobs[i].end=b+1;
